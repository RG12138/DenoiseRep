...
class CUHK03(BaseImageDataset):
    ...
    def __init__(self, root='data', split_id=0, cuhk03_labeled=False, cuhk03_classic_split=False, verbose=True, **kwargs):
        ...
        # self.train = np.hstack((train, np.zeros([len(train), 1])))
        # self.query = np.hstack((query, np.zeros([len(query), 1])))
        # self.gallery = np.hstack((gallery, np.zeros([len(gallery), 1])))
        self.padding(train)
        self.padding(query)
        self.padding(gallery)
